...
if (enc.encryptRSA(text, encryptedText)) {
        ...
        if (enc.decryptRSA(encryptedText, decryptedText)) {
                logstream << "Result of decrypt:" << decryptedText.c_str() <<
endl << "It's equal to original:" << (text == decryptedText); } else { logstream
<< "Error: " << enc.errorText().c_str();
        }
} else {
        logstream << "Error: " << enc.errorText().c_str();
}
...
class Encryptor
{
  public:
    ...
    void init(ENCRYPTION_METHOD method, bool enableBase64 = true,
              bool generateKeys = false, size_t keySize = 32);
    ...
    // //
    bool setKeyAES(
        const std::string& keyString); // Key must be 32-byte size (256 bit),
    ...
    bool setKeyPairRSA(const std::string& pubKeyFile,
                       const std::string& privKeyFile);
    bool createKeyPairRSA(const std::string& pubKeyFile,
                          const std::string& privKeyFile);

    bool encryptRSA(const std::string& input, std::string& output);
    bool decryptRSA(const std::string& input, std::string& output);

    bool encryptFileRSA(const std::string& inputPath,
                        const std::string& outputPath);
    bool decryptFileRSA(const std::string& inputPath,
                        const std::string& outputPath);
    ...
    bool encryptChaCha20(const std::string& plaintext, const std::string& iv,
                         std::string& encryptedText);
    bool decryptChaCha20(const std::string& encryptedText,
                         const std::string& iv, std::string& decryptedText);
  ...
  protected:
    ...
    bool aesEncrypt(const std::string& plaintext, const std::string& key,
    ...
    // Возвращает true в случае успеха и false в случае ошибки
    bool aesDecrypt(std::string& ciphertext, const std::string& key,
